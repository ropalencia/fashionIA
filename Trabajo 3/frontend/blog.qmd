---
title: "Prueba de concepto de FashionGenius (Recomendación de moda)"
...
display(SVG('Requisitosfuncionales.svg'))
#C:\Users\DELL\Documents\GitHub\fashionIA\Datos\Requisitosfuncionales.svg
```
...
display(SVG('Requisitosnofuncionales.svg'))
```
# Diagrama de arquitectura

### Este diagrama muestra la arquitectura de un sistema en AWS diseñado para proporcionar recomendaciones de moda basadas en un modelo alojado en SageMaker.

1. Estructura General:

    El front-end está alojado en GitHub Pages, permitiendo a los usuarios acceder al sistema desde sus dispositivos (desktop o móviles).
    Cuando un usuario interactúa con la aplicación, las solicitudes pasan a través de un AWS Web Application Firewall (WAF), que filtra el tráfico malicioso antes de llegar al API Gateway.

2. Procesamiento y Escalabilidad:

    El API Gateway gestiona las solicitudes entrantes y las dirige a una función AWS Lambda, que se encarga del procesamiento backend.
    La función Lambda es escalable y puede manejar picos en la carga de trabajo automáticamente según la demanda.

3. Modelo y Base de Datos en una Subred Privada:

    El modelo de recomendación está alojado en Amazon SageMaker y la base de datos relacional está en Amazon RDS.
    Ambos servicios están dentro de una subred privada en la VPC, lo que garantiza que no se pueda acceder a ellos directamente desde la Internet pública, aumentando así la seguridad.

4. Monitoreo y Alarma:

    Todo el sistema está monitoreado por Amazon CloudWatch, con alarmas previamente configuradas para detectar y responder a problemas en tiempo real.

```{python}
from IPython.display import SVG, display

display(SVG('FashionArchitecture.svg'))
...
```{python}
#styles_df = pd.read_csv("C:/Users/DELL/Documents/GitHub/fashionIA/Datos/styles.csv", on_bad_lines='skip')
...
En esta etapa del proyecto, se configura la arquitectura del modelo utilizando la red preentrenada VGG16, conocida por su eficacia en tareas de visión por computadora. Primero, se carga el modelo VGG16[@NLPFashion] excluyendo sus capas finales, ya que solo se necesitan las capas convolucionales que extraen características de las imágenes. Luego, se crea un modelo secuencial en el que se añaden todas las capas de VGG16. Para finalizar, se incorpora una capa de GlobalAveragePooling2D [@DeepLearningBook], que reduce la dimensionalidad de las características extraídas, preparando el modelo para la tarea específica que se va a abordar. Este proceso permite aprovechar el aprendizaje previo de VGG16 mientras se adapta a nuevas entradas. Toda esta configuración se puede revisar en detalle en el script disponible en el repositorio
...
A continuación, el modelo K-Nearest Neighbors (KNN) [@DeepLearningBook] se utiliza para encontrar las imágenes más similares en el conjunto de datos, basándose en las características procesadas de la imagen de entrada. Finalmente, se visualiza la imagen proporcionada por el usuario junto con las imágenes de productos más similares encontradas en el dataset, proporcionando una recomendación visual y personalizada. Esta función es clave para implementar un sistema de recomendación que pueda sugerir productos similares a los que un usuario podría estar interesado, mejorando así la experiencia del usuario en aplicaciones de comercio electrónico o catálogos de productos.
...
def recommend_similar_products(input_image_path, model, pca, neigh, val_sampled, num_recommendations=5):
    ...
    dist, index = neigh.kneighbors(img_features_pca[0][:313].reshape(1, -1))
...
Se observa que el modelo presenta errores en sus predicciones, lo cual se atribuye al hecho de que fue entrenado con una muestra muy pequeña(1000) debido a limitaciones computacionales. Sin embargo, se espera que al aumentar el tamaño de la muestra, estos errores se reduzcan significativamente, mejorando la precisión del modelo.
...
Desarrollo: Evidencias de pruebas funcionales, Conclusiónes, Referencias, estructurar el blog y publicarlo


* Junior Antonio Muños Henao

Desarrollo: La introducción, Análisis de requisitos del sistema (alto nivel) y
Diagrama de arquitectura
